...
function installDemoStatic(app, root = path.resolve(__dirname, '../../../..')) {
  ...
  const page = name => {
    ...
    const document = /<body[\s>]/i.test(html)
      ? html.replace(/<body([^>]*)>/i, `<body$1>${notice}`)
      : `<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Nitewide Admin</title></head><body>${notice}${html}</body></html>`;
