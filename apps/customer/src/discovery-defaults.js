...
export function formatCity(location) {
  ...
  return region && region.toLowerCase() !== city.toLowerCase() ? `${city}, ${region}` : city;
}
...
export async function detectCurrentCity({ fetchImpl = fetch, geolocation = globalThis.navigator?.geolocation } = {}) {
