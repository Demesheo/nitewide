...
- **Navigation:** a thumb-reachable bottom bar exposes the same authorized pages as desktop. Team remains restricted to authorized managers/owners. The header menu opens a Radix dialog with workspace/account details, focus trapping, Escape dismissal, and focus return. Switching pages starts at the top on mobile. Resizing to desktop closes the drawer.
- **Data:** KPI cards use two columns. Event, attendee, purchase, guestlist, and team tables become labeled cards at 600px or narrower. `MobileTableSort` retains the original sort state, default direction, and all columns. Pagination remains 10/25/50. Dense financial/drill-down reports retain their table structure with horizontal scrolling and a sticky first column.
- **Forms:** 44px primary controls, 16px input text, constrained popovers, wrapped filters, and single-column invitation forms. Event creation/editing becomes a full-height sheet with a separately scrolling body and persistent Back/Continue/Save footer. Tier pricing, inventory, windows, images, and access controls are unchanged.
- **Device behavior:** safe-area insets protect controls from iPhone notches/home indicators; dynamic viewport heights accommodate browser chrome. Pinch zoom remains enabled. Reduced-motion preferences are respected. Smaller dialogs scroll internally instead of extending beyond the screen.

Run `npm test --workspace @nitewide/business` and `npm run build --workspace @nitewide/business`. `test/mobile-layout.test.js` checks rendered sort controls and their callbacks, responsive CSS scoping, cell labels/pagination, and shared authorized navigation. These are component/contract tests, not an iOS browser emulator.

Responsive browser QA checklist (also repeat on a physical iPhone/Safari before release):

1. Test 375×667, 390×844, 430×932, 844×390 landscape, and 1440×1000 desktop. No document-level horizontal overflow; wide financial tables scroll within their own wrappers.
2. Sign in as a portfolio owner and a basic employee. Verify authorized destinations, organization/venue filters, and own-data restrictions; open/close the menu, use Escape, and resize with it open.
3. Sort a team roster by sales, change direction, paginate, and open/close a member dialog. Confirm every labeled value matches desktop.
4. Open an upcoming event; walk all three editing steps without saving. Reach ticket inventory, windows, predecessor selectors, and the footer on a short viewport. Cancel without changing event data.
5. Open an event's Guestlist subtab, change request status, open a request, and check that permitted review actions and allocation controls remain reachable. Do not approve/decline real requests merely to test layout.
6. Check Overview/Analytics charts, custom date inputs, multiselect positioning, and report drill-down. On physical Safari also check the virtual keyboard, native date picker, notch/home indicator, and VoiceOver.

September 22, 2026 verification used the in-app Chromium browser at the sizes above. Physical iOS Safari, virtual-keyboard behavior, and VoiceOver still need device QA; viewport testing is not a substitute.
...
**Personal workspace:** a basic venue employee or event/venue promoter gets distinct Overview, My analytics, and My events navigation; guestlist requests are under each authorized event's Guestlist subtab. There is no Organization page, organization-management control, full-venue revenue panel, or event editor. Their sales and customers are limited to purchases credited to their referral assignments, and their guestlist requests are limited to their own referrals. Employees can open events at their venue; an event-only promoter can open only assigned events. Both can retain their own historical credited sales after an event assignment is deactivated while current venue membership still authorizes event access. Owners, venue managers, independent event creators, and internal admins keep management views for the organizations or events they actually manage. A user with both management and basic roles sees authorized management data as well as their own attributed performance. The API enforces these boundaries independently of the UI.

**Guestlist invitations and notifications:** authorized users can invite by email or phone from the Share this event card in Event details, using the direct pool or their own positive allocation as their role permits. Existing matching customers are confirmed immediately; new guests get a private signup link to share manually and capacity is checked when claimed. Business guestlist notifications open the relevant event's Guestlist subtab and request. Both Business and Customer include an in-app notification inbox. See [invitation behavior, API, verification and future email/SMS delivery plan](GUESTLIST_INVITES_NOTIFICATIONS.md).
...
**Event Guestlist subtab:** open any authorized event under Events, then select Guestlist. The standalone Guestlists navigation has been removed. The Share this event card above the tabs provides the signed-in user's unique referral link and private guest invitations. Choose one or more request statuses (or clear the selection for all). The table shows guest names but not email addresses; click a name for a details dialog with contact information, event, party, referral, review, and check-in details. The dialog handles pending approvals/declines and a two-step cancellation of approved, unused entries. Cancelling revokes the QR credential, records an audit event, and immediately releases the direct or individual referral allocation. Source shows Direct or the referrer's name; there is no separate referral column. Checked-in status becomes available 24 hours before the event starts, matching the Admissions window; no-show stays unavailable before start. Only approval issues a credential. Owners/managers, independent event creators, and internal admins can review any request for their event; a basic employee or promoter can view and review only entries referred by their own active EventAffiliate. They cannot approve, decline, or cancel a Direct entry or another person's referral. Owners and managers can set direct-event capacity and each selected referrer's additional allocation. Each capacity card keeps its number in the same input; **Edit** unlocks that field and replaces Edit with the standard Save and Cancel actions for that pool. Blank promoter limits inherit the organization default when saved. Limits cannot be lowered below already approved/checked-in party members. A direct 50 plus two promoter 20s remains 90 possible guest admissions. Past events keep their request and allocation history visible, but allocations are read-only.

**Team:** owner and manager accounts see only organizations they manage. The **Invite team member** button opens a dialog for email, optional phone, and role. The role menu uses the shared accessible dark select, with high-contrast menu and highlighted options. Owners and managers can invite other managers, employees, or promoters; employees cannot invite or edit events. A newly created invite link stays in the dialog for copying. New invitees register as customers first, while existing customers retain their identity and other roles. Invitation tokens are stored only as hashes, expire after seven days, and can be accepted only by the matching email. The Team table lists owners, managers, employees, and promoters; clicking a person opens a modal with their profile and 30-day referred-sales figures. Owners and managers use **Edit member** to change another non-owner's organization role to manager, employee, or promoter, or remove the member after confirmation. Role changes and removals are audited; a removal disables organization access and organization-linked referrals while event-only promoter assignments remain separate. Owners cannot be reassigned or removed here, and users cannot change or remove their own role. On phones, the member modal fits the safe-area viewport and scrolls internally; metrics collapse to compact cards and edit/removal actions become full-width touch targets. The destructive **Remove member** action sits below Save/Cancel behind added spacing and a divider, reducing accidental taps. Tapping it opens a separate compact confirmation dialog centered in the viewport, rather than scrolling the member dialog. The modal closes with its Close button, the X, Escape, or a click outside, returning focus to the selected member (or the invite action if that row was removed). Pending invitations can be deleted or renewed by an owner or manager who manages the organization. **Resend** opens a prefilled email in the operator's email app; Nitewide does not yet deliver email automatically. Owners/managers should copy the newly generated link if no email app opens.
...
- Business event summaries show face-value Total sales, recorded Commissions, Paid orders, and Check-ins / expected (ticket admissions plus approved guestlist spots). Business API responses omit customer-paid totals and Nitewide platform-fee totals, including nested customer rows. Only internal Admin reports receive those aggregate fields; customers can still see their own checkout total and receipt.
...
- Orders count checkouts; units count purchased tier quantities; expected admissions use active ticket credentials (with historical `entriesPerUnitSnapshot` as a legacy report fallback). Packages can admit multiple people. Actual door attendance counts checked-in tickets plus checked-in guestlist party members.
...
Event writes use a serializable transaction, lock the event before offerings (same order as checkout), validate tier ownership, and record AuditLog before/after values. Unsold tiers with no order history can be removed on Save, including the final tier. Sold or historically purchased tiers must be deactivated instead; the server returns TIER_HAS_SALES if deletion is attempted. The removal confirmation warns about dependent opening rules, which are cleared before saving. Inventory cannot drop below units sold. The type/admissions of a sold tier cannot change. Historical OrderItem prices remain immutable. Organization reassignment is forbidden. Stale versions and concurrent writes return 409 with a reload/retry message. UI must not automatically replay a stale save.
...
| `src/components/Guestlists.jsx`    | Event-scoped request review, status filters, direct/promoter limits                                                        |
