...
function createPublicController({ models }) {
  return {
    listEvents: async (req, res) => {
      ...
      const events = await models.Event.findAll({ where, include: [{ model: models.Location, as: 'location' }, { model: models.Organization, as: 'organization', attributes: ['id', 'name', 'slug', 'planTier'] }, { model: models.Offering, as: 'offerings', required: false }], order: [['startsAt', 'ASC']], limit: Math.min(Number(req.query.limit) || 50, 100) });
