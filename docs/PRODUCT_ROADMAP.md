...
- Buyers pay the same **standard 8% + $0.80 (automatic discounts and minimum-cost exceptions apply) service fee per paid ticket/package** for Free and Premium organizations; under the current approved policy, **Nitewide pays routine payment-processing costs**. There is no additional Nitewide organizer listing/transaction fee. Premium is optional at $249/month and does not reduce transaction fees; its value is advanced analytics, promoter ROI, CRM/marketing, and business-management capability. Advanced ticket/package configuration remains available on Free. See [current fee policy](FEE_POLICY.md); actual provider costs must be reconciled from provider records, not an estimate.
...
| M1 — Transaction-ready alpha | Sep 28–Oct 18, 2026 | A customer can discover, register, purchase in Stripe Connect test mode, receive an opaque signed QR credential, request guestlist access, and be checked in through an auditable payment-to-admission chain; business/admin users operate and reconcile the complete flow. |
...
| Sprint 1 | Sep 28–Oct 4 | Managed identity, event detail/offer selection, organization/event CRUD, Stripe Connect account/charge/liability ADR, sandbox foundation, and staging deployment. |
...
Stripe Connect is the selected MVP processor because it provides the clearest and fastest implementation path for the solo-founder launch. Nitewide will use Stripe-hosted or embedded onboarding, connected-account payment components, direct charges on the actual seller's connected account where the approved configuration permits, and Stripe balance transactions as the settlement source of truth.

Stax Connect remains a documented post-launch commercial optimization and Orlando partnership opportunity. It is not required for alpha or Florida launch. Revisit it when real processing volume, conversion, dispute, payout, and support data can support a credible negotiated comparison against Stripe.

Build Stripe-specific adapters behind clear payment, ledger, dispute, and payout service boundaries without delaying delivery for premature multi-provider abstraction. The active policy makes Nitewide the routine Stripe processing cost bearer. This does not insure venue risk or decide merchant-of-record or loss liability. Before production, confirm a supported platform-paid processing configuration that preserves the intended seller and approved loss-liability requirements.
...
Decision sources: [payment provider evaluation](PAYMENT_PROVIDER_EVALUATION.md), [Stax company overview and Orlando headquarters](https://staxpayments.com/about-stax/), [Stax Connect](https://staxpayments.com/stax-connect/), [Stax split funding](https://docs.staxpayments.com/docs/split-funding-1), [POSH balance and payout documentation](https://support.posh.vip/en/articles/15090213-managing-your-balance-payouts-bank-accounts), [Stripe Connect payouts](https://docs.stripe.com/connect/supported-embedded-components/payouts), and [PayPal Complete Payments Platform](https://developer.paypal.com/platforms/get-started/).

## Buyer-funded checkout and promoter reward economics

September 23 follow-up: Competitive discounts and minimum-cost adjustments now run in the demo. Standard fees are 8% + $0.80 per paid unit, with a hard $1 contribution after modeled costs and a preferred 5% target. The floor overrides both the standard fee and competitive target. Purchases are not rejected solely because of pricing targets. See [pricing design](MARGIN_PROTECTED_PRICING_DESIGN.md). Live provider verification and reconciliation remain planned.

The buyer-facing service fee is gross Nitewide platform revenue from which Nitewide covers routine Stripe processing. Checkout must show the ticket subtotal, Nitewide service fee, taxes, discounts/credits, and final customer total before payment. Nitewide's actual Stripe processing cost is recorded separately in internal settlement and admin reporting and is never added as a second customer surcharge.

- Use one transaction-fee schedule for both organization plans: the buyer pays standard 8% + $0.80 (automatic discounts and minimum-cost exceptions apply) per paid ticket/package. Premium costs $249/month and provides no transaction-fee reduction. Do not add an organizer-side Nitewide transaction fee or silently double-charge both sides.
- Apply both components per paid ticket/package unit, rounding each unit's percentage before multiplying by quantity. Free units incur no fee; package admission count does not increase the fixed fee.
- On a $20 ticket, the buyer fee is $2.40 and checkout is $22.40. Estimated Stripe processing is $0.95 at 2.9% + $0.30 of full checkout. Venue proceeds are $20 before commissions/other obligations; Nitewide retains $1.45 after modeled card processing, before other costs. Actual settlement requires provider records. See [fee policy](FEE_POLICY.md).
- Report Nitewide's Stripe processing cost and gross application-fee revenue separately in admin-only financial reporting; do not describe gross application fees as net cash flow. The ledger must make the responsible party explicit for every charge.
...
The current mature-market planning case assumes promoter or affiliate attribution on 50% of paid transactions. Attribution share is independent from the 90%/10% split of the organizer-funded reward and must be replaced with reconciled actual cohort data as markets launch.

- Example: a $5.00 gross promoter reward would produce a planned $0.50 Nitewide reward fee and $4.50 net reward before disclosed adjustments. This proposed income is separate from buyer service fees and is not used to satisfy checkout contribution floors. Routine Stripe processing is Nitewide-paid; refunds and disputes may reduce realized contribution.
...
- Maintain the pricing source of truth: buyers pay standard 8% + $0.80 (automatic discounts and minimum-cost exceptions apply) per paid ticket/package regardless of whether the organization is Free or Premium; Premium is $249/month with no transaction-fee reduction. Keep affiliate service fees separately versioned. Report subscription billings, credits, billing-provider costs, gross platform fees, refunded/credited fees, processor costs, affiliate fees/rewards, dispute losses, reserves, and net revenue separately rather than presenting revenue as pure profit.
...
| ~$25M–$50M growth case | Validate the reproducible Florida operating model: target 80 retained active nightclub venues producing $500 of GA sales per event-night, plus one $5,000 ticket-only non-nightclub event per ten nightclub event-nights, full payment on every sale, a $400 blended VIP checkout, 50% promoter/affiliate-attributed transactions, ~$44.4M total face-value GMV, ~$4.6M gross platform revenue including privacy-safe contextual ads and aggregated insights, reconciled positive contribution, durable venue/organizer/buyer/promoter cohorts, Premium usage, low concentration, and a repeatable national playbook. A $50M outcome still requires a premium ~10.8x strategic multiple or more scale; it is not inferred from one comparable. |
