...
# Euphoria duplicate repair

The Euphoria organization was not duplicated. Its addressless legacy demo draft and imported locations caused duplicate venue options; a generic Saturday event also overlapped the September 26 Rew1nd listing. Run `node apps/api/src/db/reconcile-euphoria.js --dry-run`, then `--apply` against the local demo database only.

The repair uses Euphoria's saved address, 110 S Orange Ave, for its matching event locations. It retains old Location rows for recovery. For the single known Saturday collision it preserves the booked event UUID/slug, orders, ticket credentials, inventory, commissions and guestlist entries, adopting the imported flyer/text/end time. It only removes the unpurchased source copy if every guest is an exact duplicate by customer, party size, status and referring person, and all active referrers retain access. Orders, attendance, invites, notifications, boosts, sold inventory, non-guestlist attribution, or unmatched guests abort the entire transaction. The removed source event and all its child rows are archived in `event.demo_duplicate_consolidated`; location changes receive separate audits. Recovery requires restoring that audit archive transactionally, not a UI undo.

The repair is repeatable and runs before the Orlando refresh. Both Posh snapshots now enable physical-venue overlap checks, preventing reimport of the removed occurrence. Existing bookings are never deleted by this repair. Tests: `node --test apps/api/test/euphoria-reconciliation.test.js`.
