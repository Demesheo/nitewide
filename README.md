...
For the **September 22 Orlando expansion and purchase cleanup**, preview with `npm run db:seed:orlando -- --dry-run`, then apply with `npm run db:seed:orlando -- --apply`. This deliberately removes only conflicting `seed`-provider paid orders, archiving their records in the audit log and restoring inventory. Guestlists may overlap both purchases and other guestlists and are never removed by this cleanup. Browser-created `demo` purchases are protected. New seed purchases select a non-conflicting customer (or create another demo customer); the live checkout policy is unchanged. Room 22 retains its own event location but shares Proper's organization, owners, managers and staff. The import adds verified listings for Elixir, Sessions, Taco Kat, The Robinson Cocktail Room and McQueens, with demo teams and schedule-safe promoters. See [refresh scope, recovery and source notes](docs/ORLANDO_SEED_REFRESH.md).

Additional demo credentials use the existing password **`NitewideDemo!2026`**. For each slug `elixir`, `sessions`, `taco-kat`, `robinson`, or `mcqueens`: owner **`<slug>.owner1@nitewide.test`**, manager **`<slug>.manager1@nitewide.test`**, employee **`<slug>.employee1@nitewide.test`**. Numbered additional managers/employees use the same pattern. Extra schedule-available promoters use **`orlando.expansion.promoter<N>@nitewide.test`**; additional purchasing customers use **`orlando.customer<N>@nitewide.test`**. These are local-only fixture accounts, not real venue staff. Existing Proper and Room 22 manager credentials continue to work for their shared organization.
...
The customer app provides working **Sign in** and **Create account** flows. Create account requires an exact **Confirm password** match before submission; the confirmation is client-side only and is never sent to the API or stored. The API still enforces password strength. Public registration always creates only a customer identity; clients cannot request owner, manager, employee, promoter, or administrator access. Because Nitewide uses one identity, an elevated demo user can still use the customer app as a customer. Owners can invite managers; owners and managers can invite employees or promoters from the Business Organization page. Invitees register or sign in with the invited email and accept a seven-day link. Resend renews the link and opens a prefilled email in the operator's mail app; automatic delivery is not implemented yet.
...
- [Technical whitepaper](docs/investors/Nitewide_Technical_Whitepaper.docx) — current capabilities, planned infrastructure, roadmap, and margin-protected economics.

These investor/partner materials include internal unit economics. Do not publish them as customer-facing pricing disclosures. Planned capabilities and market-share scenarios are not production availability or actual traction.
