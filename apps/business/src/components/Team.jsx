...
export function Team({ session, organizations, onUnauthorized }) {
  ...
  async function submit(event) {
    ...
    try {
      const result = await api(`/business/organizations/${organizationId}/invitations`, session, { method: 'POST', body: JSON.stringify({ email, role }) });
...
export function TeamInviteLanding({ token, session, onSession, onAccepted }) {
  ...
  async function submit(event) {
    ...
    try {
      const currentSession = await api(register ? '/auth/register' : '/auth/sign-in', null, { method: 'POST', body: JSON.stringify(register ? { displayName: form.get('name'), email: form.get('email'), password: form.get('password'), marketingConsent: false } : { email: form.get('email'), password: form.get('password') }) });
