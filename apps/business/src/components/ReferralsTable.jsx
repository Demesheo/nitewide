...
export function ReferralsTable({ data }) {
  ...
  return <section className="rounded-xl border border-border bg-card">
    ...
    <div className="table-wrap"><table><thead>{!person ? <tr>{head('Team member', 'label')}{head('Role', 'role')}{head('Paid orders', 'orders')}{head('Referred customers', 'customers')}{head('Sales value', 'salesCents')}{head('Guestlist places', 'guestlistPlaces')}{head('Recorded commission', 'commissionCents')}</tr> : <tr>{head('Customer', 'label')}{head('Paid orders', 'orders')}{head('Sales value', 'salesCents')}{head('Guestlist places', 'guestlistPlaces')}{head('Avg. order', 'averageOrderCents')}</tr>}</thead><tbody>{!person ? pager.rows.map((row) => <tr key={row.id} className="cursor-pointer hover:bg-accent/50" onClick={() => setPersonId(row.id)}><td><button type="button" className="analytics-drill" onClick={(event) => { event.stopPropagation(); setPersonId(row.id); }}>{row.label}<ChevronRight size={13}/></button></td><td>{row.role}</td><td>{row.orders}</td><td>{row.customers}</td><td>{money(row.salesCents)}</td><td>{row.guestlistPlaces || 0}<small className="block text-muted-foreground">{row.approvedGuestlistPlaces || 0} approved</small></td><td>{money(row.commissionCents)}</td></tr>) : pager.rows.map((row) => <tr key={row.id}><td><strong>{row.label}</strong>{row.email && <small className="block text-muted-foreground">{row.email}</small>}</td><td>{row.orders}</td><td>{money(row.salesCents)}</td><td>{row.guestlistPlaces || 0}<small className="block text-muted-foreground">{row.approvedGuestlistPlaces || 0} approved</small></td><td>{money(row.averageOrderCents)}</td></tr>)}</tbody></table></div>
