...
export function ExplorerTable({ data }) {
  ...
  return <section className="rounded-xl border border-border bg-card">
    <div className="flex flex-wrap items-center justify-between gap-3 p-5">
      <div><span className="eyebrow">EXPLORE PERFORMANCE</span><h3 className="text-lg font-semibold">{levelNames[level]}</h3><p className="text-xs text-muted-foreground">{level === 'customer' ? 'Customers with paid orders or approved guestlist entries. Use this information for event operations only.' : 'Select a row to see the next level.'}</p></div>
