...
function fixture({ sold = 0, total = 5 } = {}) {
  ...
  const created = { tickets: 0 }; const tx = { LOCK: { UPDATE: 'UPDATE' } };
  const models = {
    ...
    Payment: { create: async () => ({}) }, AffiliateAttribution: { create: async () => ({}) }, AuditLog: { create: async () => ({}) },
  };
  const sequelize = { transaction: async (_options, work) => work(tx) };
  return { checkout: createCheckoutService({ sequelize, models }), getIncrements: () => increments, created };
}
test('checkout snapshots a sale, increments inventory, and creates credentials', async () => {
  ...
  assert.equal(result.order.subtotalCents, 4000); assert.equal(result.order.platformFeeCents, 409); assert.equal(f.getIncrements(), 2); assert.equal(result.credentials.length, 2);
});
