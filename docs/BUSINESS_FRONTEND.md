...
**Overview:** select one or several authorized organizations, or independent events and a 7/30/90/365-day sales period. Review gross sales, paid orders, average order, recorded commissions, daily sales, sales by event/tier, and team/promoter performance. The performance table has a multi-select Roles dropdown for owners, managers, employees, and promoters; no selection shows everyone. The Sales mix tabs show a face-value revenue pie with a top-six-plus-Other legend and ranked amounts for tickets/packages or events. The organization picker is hidden when only one workspace is authorized. Refresh explicitly reloads current API data. Export downloads all report rows and daily values as CSV; formula-like names are escaped to avoid spreadsheet injection.
...
**Events:** search titles/cities/categories and filter by status. Authorized events have Edit actions; promoter-only events are read-only. Create uses a three-step dialog: essentials, location/access, and offerings/publication. Independent events belong to the signed-in creator. Organization options include only those the user can manage. Drafts stay out of discovery; publication and discoverability are separate controls.

The editor supports generic categories, venue-local dates, IANA time zones, address privacy, event guestlist capacity, reference venue capacity, multiple ticket/package/reservation tiers, prices, finite/unlimited inventory, admissions per unit, min/max quantities, sales windows, active state, and public/hidden visibility. Existing password tiers can retain their visibility, but provisioning passwords is outside this UI. New events start with $10 GA and $300/$400/$1,000 packages; all prices are editable and in USD.

**Guestlists:** choose an event and request status, then approve/decline pending requests. Only approval issues a credential. Owners, managers, and employees can approve; owners and managers can set direct-event capacity and each selected promoter's additional allocation. Blank promoter limits inherit the organization default. Limits cannot be lowered below already approved/checked-in party members. A direct 50 plus two promoter 20s remains 90 possible guest admissions. The current approval policy allows active organization/event promoters to review the event queue; it is not restricted to only their own referred guests.
...
| View complete event sales             | Any event        | Their organization's events | Their own event                   | Own attributed orders only              | No                 |
| Review guestlist                      | Any event        | Their organization's events | Their own event                   | Active org/event relationship           | No                 |
...
Locations are copy-on-write so editing one event cannot change every event sharing a venue location. Address edits do not automatically geocode; coordinates need a future geocoding workflow. UTC timestamps are derived from the entered location time zone, independent of the browser zone. Nonexistent spring-forward local times are rejected; ambiguous fall-back times use the converter's first matching occurrence—use a non-ambiguous time around clock changes until explicit offset selection is added.
