...
export function SalesMixPie({ slices }) {
  ...
  return (
    <div className="mix-visual">
      <div className="mix-pie" role="img" aria-label={`Sales mix by face-value revenue: ${slices.map((row) => `${row.name} ${money(row.salesCents)}`).join(', ')}`}>
        <ResponsiveContainer width="100%" height="100%">
          <PieChart>
            <Pie data={slices} dataKey="salesCents" nameKey="name" innerRadius={53} outerRadius={78} paddingAngle={2} stroke="none" isAnimationActive={false}>
      ...
      <div className="mix-legend" aria-label="Sales mix legend">
        {slices.map((row, index) => <div key={row.id}><span className="mix-swatch" style={{ backgroundColor: mixColors[index % mixColors.length] }}/><span className="mix-legend-name" title={row.name}>{row.name}</span><strong>{Math.round((row.salesCents / total) * 100)}%</strong></div>)}
      </div>
    </div>
  );
}
