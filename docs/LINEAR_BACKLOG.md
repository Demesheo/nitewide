...
- Synchronized through September 21, 2026 as 104 issues, `DEM-5` through `DEM-108`, with no missing identifiers or duplicate titles.
- The original 53 issues were expanded in place; 51 operating, legal, finance, payment, admission-evidence, dispute-risk, affiliate-economics, expansion, communications, and marketing issues were added.
- All 104 synchronized issues are intentionally unassigned and carry a project, milestone, priority, relevant labels, testable acceptance criteria, verification evidence, dependencies/risks, and a solo-founder execution note; estimates are retained where assigned.
- Current synchronized milestone allocation is M0: 12, M1: 28, M2: 21, M3: 12, M4: 14, M5: 9, and M6: 8. No synchronized issue is missing a milestone.
- Stripe Connect is now the committed MVP path. `PAY-01`, `PAY-06`, `PAY-07`, the project description, and M0/M1 descriptions were updated for organizer-paid Stripe processing. `DEM-108` is now the low-priority M4 issue `[PAY-13] Revisit Stax partnership after Stripe launch`.
- `ADS-01` and `DATA-02` are approved locally and queued for the next Linear synchronization because the workspace's Linear Agent daily limit was reached. Once synchronized, the expected total is 106 issues and M4 increases to 16.
...
| PAY-01 | 5 | P0 | As finance, I need a Stripe Connect architecture decision so fees, refunds, payouts, tax, and reconciliation have one source of truth. | Stripe Connect is selected for MVP; direct-charge/account model, organizer-paid processing, application fees, money flow, webhooks, idempotency, ledger, dispute/refund/payout boundaries, and sandbox plan are approved. |
...
| ID | Pri | Pts | Deliverable and acceptance summary |
|---|---:|---:|---|
| PAY-06 | P0 | 8 | Complete the Stripe Connect proof of concept and ADR. Target direct charges on the venue/organizer seller's connected account, organizer-paid Stripe processing, Nitewide application fees, Stripe-owned payment-loss liability where available, negative-balance recovery, dispute evidence, reconciliation, payout states, wallets, and explicit prohibited platform-liability configurations. |
| PAY-13 | P2 | 5 | After launch, revisit a 10–20 venue Orlando Stax Connect design-partner and cost-optimization proposal using reconciled Stripe volume and operating data. Require materially better all-in economics without weaker liability, wallet, reconciliation, security, portability, or support terms. This is not an MVP dependency. |
...
| ECON-01 | P0 | 8 | Implement versioned pricing: customers pay 7.5% + $0.79 per paid order for Free and optional $249/month Premium; organizations/creators pay Stripe processing, with no additional Nitewide organizer listing/transaction fee or Premium fee discount. Preserve explicit fee display, configurable 10% service fee on organizer-funded promoter rewards, gross/fee/net ledgers, refunds/reversals and unit-economics telemetry. Record processing payer in snapshots, use actual Stripe balance transactions for settlement, and distinguish customer savings from organizer costs. Never call an estimate the exact Stripe fee or report gross fees/subscriptions as net profit. Local policy updated September 21; remote Linear synchronization remains a follow-up. |
