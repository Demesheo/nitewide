...
export function EventTable({ rows, columns, defaultSort = 'name', defaultDescending = false, empty = 'No matching records', emptyDescription = 'Records appear here as activity is recorded for this event.', onSelect, onPageChange, selectRow = false, searchable = true }) {
  ...
  return <>{filtered.length > 0 && <MobileTableSort columns={columns} value={sort} descending={descending} onChange={(key) => { setSort(key); setDescending(Boolean(columns.find(c => c.key === key)?.numeric)); }} onToggle={() => setDescending(!descending)}/>}
    {searchable && <div className="toolbar"><div className="search-field"><Search size={16}/><Input aria-label="Search table" placeholder="Search" value={search} onChange={(e) => setSearch(e.target.value)}/></div></div>}
    {filtered.length ? <><div className="table-wrap responsive-event-table"><table><thead><tr>{columns.map((c) => <th key={c.key} scope="col" className={c.className || ''} aria-sort={sort === c.key ? descending ? "descending" : "ascending" : "none"}><button type="button" className="analytics-sort" onClick={() => { setDescending(sort === c.key ? !descending : Boolean(c.numeric)); setSort(c.key); }}>{c.label}<span aria-hidden="true">{sort === c.key ? descending ? ' ↓' : ' ↑' : ' ↕'}</span></button></th>)}</tr></thead><tbody>{pager.rows.map((row) => <tr key={row.id || row.userId || row.name} className={selectRow && onSelect ? 'event-selectable-row' : undefined} onClick={selectRow && onSelect ? (e) => { if (!e.target.closest('button, a, input, select, textarea')) onSelect(row); } : undefined}>{columns.map((c, i) => <td key={c.key} data-label={c.label} className={[c.className, c.numeric ? 'numeric' : ''].filter(Boolean).join(' ')}>{i === 0 && onSelect ? <button className="event-text-link" onClick={() => onSelect(row)}>{c.render ? c.render(row) : row[c.key]}</button> : c.render ? c.render(row) : row[c.key]}</td>)}</tr>)}</tbody></table></div><TablePagination pager={pager} onPageChange={onPageChange}/></> : <Empty title={empty}>{emptyDescription}</Empty>}
  </>;
}
