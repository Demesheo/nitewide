      ...
      <Tabs value={tab} onValueChange={(value) => { setTab(value); setTicket(null); setError(''); }}>
        ...
        <TabsContent value="plans">
          {ticket ? <section className="ticket-view">
            ...
            <div className="purchase-ticket-heading"><EventArtwork event={ticket.event} className="booking-flyer" /><div><p className="eyebrow">{ticket.demo ? 'DEMO PURCHASE' : 'YOUR NIGHT, CONFIRMED'}</p><h3>{ticket.event.title}</h3><p>{eventDate(ticket.event)} · {eventTime(ticket.event)}</p><p>{ticket.event.location?.name || ticket.event.organization?.name}</p><p aria-label="Event address">{eventAddress(ticket.event.location)}</p></div></div>
