...
const NAV = [['overview', 'Overview'], ['reports', 'Reports & analytics'], ['organizations', 'Organizations'], ['events', 'Events'], ['orders', 'Orders & payments'], ['users', 'Users'], ['audit', 'Audit log']];
const editable = {
...
function SalesChart({ rows = [] }) {
  ...
  return <div className="bars">{rows.slice(-14).map((row) => <div className="bar-cell" key={row.id} title={`${row.label}: ${formatMoney(row.salesCents)}`}><div className="bar" style={{ height: `${Math.max(4, row.salesCents / max * 100)}%` }}/><small>{row.label.slice(5)}</small></div>)}</div>;
}
...
function Dashboard({ session, onSignOut }) {
  const [section, setSection] = useState('overview'); const [days, setDays] = useState(30); const [organizationId, setOrganizationId] = useState(''); const [search, setSearch] = useState(''); const [statusFilter, setStatusFilter] = useState(''); const [data, setData] = useState(null); const [error, setError] = useState(''); const [editing, setEditing] = useState(null); const [creatingUser, setCreatingUser] = useState(false); const [mobileNav, setMobileNav] = useState(false);
  async function load() { setError(''); try { setData(await api(`/admin/workspace?days=${days}&limit=100${organizationId ? `&organizationId=${organizationId}` : ''}`)); } catch (err) { setError(err.message); } }
  useEffect(() => { load(); }, [days, organizationId]);
  ...
  function edit(kind, record) { setEditing({ kind, record }); }
  function exportCurrent() { const source = section === 'reports' ? data.sales.events : section === 'overview' ? data.sales.daily : rows[section] || []; exportCsv(`nitewide-${section}.csv`, source); }
  const cols = {
  ...
  return <div className="app-shell"><header className="topbar"><Button variant="ghost" size="icon" className="menu" onClick={() => setMobileNav(!mobileNav)}>☰</Button><a className="brand" href="#"><span>N</span>NITEWIDE <em>ADMIN</em></a><div className="top-actions"><span className="health"><i/>Platform online</span><a href="http://localhost:5173">Customer ↗</a><a href="http://localhost:5174">Business ↗</a><Button variant="outline" size="icon" className="avatar" title={session.user.email}>{initials(session.user.displayName)}</Button></div></header><aside className={mobileNav ? 'open' : ''}><div className="nav-label">COMMAND CENTER</div>{NAV.map(([id,label]) => <Button variant="ghost" key={id} className={section === id ? 'active' : ''} onClick={() => { setSection(id); setMobileNav(false); }}>{label}</Button>)}<div className="side-foot"><b>{session.user.displayName}</b><small>{session.user.email}</small><Button variant="ghost" onClick={onSignOut}>Sign out</Button></div></aside><main className="workspace"><div className="page-head"><div><p className="eyebrow">NITEWIDE OPERATIONS</p><h1>{title}</h1><p>{section === 'overview' ? 'Marketplace health, sales, and action items in one accountable view.' : 'Search, inspect, export, and safely manage live platform records.'}</p></div><div className="head-actions"><select aria-label="Report period" value={days} onChange={(e) => setDays(Number(e.target.value))}><option value="7">Last 7 days</option><option value="30">Last 30 days</option><option value="90">Last 90 days</option><option value="365">Last year</option></select><select aria-label="Sales organization" value={organizationId} onChange={(e) => setOrganizationId(e.target.value)}><option value="">All organizations</option>{data?.organizations.map((org) => <option key={org.id} value={org.id}>{org.name}</option>)}</select><Button variant="outline" onClick={exportCurrent}>Export CSV</Button><Button variant="outline" onClick={() => setCreatingUser(true)}>Create demo user</Button><Button className="primary" onClick={load}>Refresh</Button></div></div>{error && <div className="error">{error}</div>}{!data ? <div className="loading">Loading platform operations…</div> : <>
    {section === 'overview' && <><section className="metrics"><Stat label="Gross sales" value={formatMoney(data.sales.summary.grossSalesCents)} note={`${data.sales.summary.orders} paid orders · ${days} days`}/><Stat label="Platform fees" value={formatMoney(data.sales.summary.platformFeesCents)} note="Customer service fees collected"/><Stat label="Organizations" value={data.stats.organizations} note={`${data.stats.premiumOrganizations} premium`}/><Stat label="Active events" value={data.stats.publishedEvents} note={`${data.stats.draftEvents} drafts`}/></section><div className="overview-grid"><section className="panel chart-panel"><div className="panel-head"><div><p className="eyebrow">SALES PULSE</p><h2>Paid volume</h2></div><strong>{formatMoney(data.sales.summary.grossSalesCents)}</strong></div><SalesChart rows={data.sales.daily}/></section><section className="panel"><div className="panel-head"><div><p className="eyebrow">OPERATIONS QUEUE</p><h2>Needs attention</h2></div></div><div className="alerts">{data.alerts.map((alert) => <button key={alert.id} onClick={() => setSection(alert.id === 'draft-events' ? 'events' : alert.id === 'organizations' ? 'organizations' : alert.id === 'failed-payments' ? 'orders' : 'overview')}><i className={alert.severity}/><span><b>{alert.label}</b><small>{alert.count ? 'Review matching records' : 'No action required'}</small></span><strong>{alert.count}</strong></button>)}</div></section></div><section className="panel"><div className="panel-head"><div><p className="eyebrow">TOP EVENTS</p><h2>Revenue leaders</h2></div><button onClick={() => setSection('reports')}>Full report →</button></div><Table rows={data.sales.events.slice(0, 5)} columns={[{ label: 'Event', key: 'label' }, { label: 'Paid orders', key: 'orders' }, { label: 'Gross sales', render: (r) => formatMoney(r.salesCents) }]}/></section></>}
    {section === 'reports' && <div className="analytics-page"><Analytics/></div>}
    {['organizations','events','orders','users','audit'].includes(section) && <section className="panel data-panel"><div className="filterbar"><label><span>⌕</span><Input value={search} onChange={(e) => setSearch(e.target.value)} placeholder={`Search ${title.toLowerCase()}…`}/></label>{section !== 'audit' && <select aria-label="Filter by status" value={statusFilter} onChange={(e) => setStatusFilter(e.target.value)}><option value="">All statuses</option>{(section === 'users' ? ['active','disabled'] : section === 'organizations' ? ['active','suspended','closed'] : section === 'events' ? ['draft','published','cancelled','completed'] : ['pending','paid','cancelled','refunded']).map((value) => <option key={value} value={value}>{value}</option>)}</select>}<span>{rows[section].length} records</span></div><Table rows={rows[section]} columns={cols[section]} kind={['audit', 'orders'].includes(section) ? null : section} onEdit={edit}/></section>}
