...
test('checkout snapshots a sale, increments inventory, and creates credentials', async () => {
  ...
  assert.equal(result.order.subtotalCents, 4000); assert.equal(result.order.platformFeeCents, 385); assert.equal(f.getIncrements(), 2); assert.equal(result.credentials.length, 2);
});
