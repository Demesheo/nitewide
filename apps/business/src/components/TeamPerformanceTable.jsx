...
function TeamRows({ people, selectedRoles, totalSales, resetToken }) {
  ...
  return <><div className="table-wrap"><table><thead><tr>{head('Team member', 'name')}{head('Role', 'role')}{head('Paid orders', 'orders')}{head('Attributed sales', 'salesCents')}{head('Guestlist places', 'guestlistPlaces')}{head('Commission', 'commissionCents')}{head('Contribution', 'contribution')}</tr></thead><tbody>{pager.rows.map((person) => <tr key={person.id}><td><div className="person"><span className="avatar">{person.name.split(' ').map((part) => part[0]).slice(0, 2).join('')}</span><strong>{person.name}</strong></div></td><td><Badge variant="outline">{person.role}</Badge></td><td>{person.orders}</td><td className="numeric">{money(person.salesCents)}</td><td>{person.guestlistPlaces || 0}<small className="block text-muted-foreground">{person.approvedGuestlistPlaces || 0} approved</small></td><td>{money(person.commissionCents)}</td><td><div className="contribution"><span style={{ width: `${totalSales ? (person.salesCents / totalSales) * 100 : 0}%` }}/></div><small>{totalSales ? Math.round((person.salesCents / totalSales) * 100) : 0}% of sales</small></td></tr>)}</tbody></table></div>{!filtered.length && <Empty title="No team members in this view">Attributed sales and guestlist activity appear when customers use referral links or codes.</Empty>}<TablePagination pager={pager}/></>;
}
