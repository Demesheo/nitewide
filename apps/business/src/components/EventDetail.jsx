...
export function EventDetail({ eventId, session, refreshToken, onBack, onEdit, onUnauthorized }) {
  ...
  return <div className="event-detail" aria-busy={loading}>
    ...
    <Tabs defaultValue="sales" className="event-detail-tabs"><TabsList aria-label="Event detail sections"><TabsTrigger value="sales">{ownOnly ? 'My sales & customers' : 'Sales overview'}</TabsTrigger><TabsTrigger value="tickets">Tickets & packages</TabsTrigger><TabsTrigger value="people">{ownOnly ? 'My referral' : 'Team'}</TabsTrigger></TabsList>
      <TabsContent value="sales"><div className="event-chart-grid"><section className="panel"><span className="eyebrow">WHAT SELLS</span><h3>{ownOnly ? 'Your sales by ticket & package' : 'Sales by ticket & package'}</h3>{s.salesCents ? <SalesMixPie slices={data.tiers.filter((t) => t.salesCents > 0)}/> : <Empty title="Sales start here">Your ticket and package mix will appear after the first purchase.</Empty>}</section>{!ownOnly && <section className="panel"><span className="eyebrow">WHO BRINGS THE CROWD</span><h3>Sales by referral channel</h3>{s.salesCents ? <SalesMixPie slices={data.channels.map((c) => ({...c,id:c.name}))}/> : <Empty title="No sales yet">Direct and referred purchases will appear here.</Empty>}</section>}</div>{!ownOnly && <section className="panel"><div className="section-heading"><div><h3>Revenue at a glance</h3><p>Recorded orders for the full event, including local demo orders. This is a sales report, not a payout balance.</p></div></div><div className="event-financials"><div><span>Customer payments, including fees</span><strong>{money(s.customerPaidCents)}</strong></div><div><span>Customer platform fees</span><strong>{money(s.platformFeeCents)}</strong></div><div><span>Recorded referral commissions</span><strong>{money(s.commissionCents)}</strong></div><div><span>Sales after referral commissions</span><strong>{money(s.salesCents - s.commissionCents)}</strong><small>Before Stripe processing and other adjustments</small></div></div></section>}<EventAttendees customers={data.customers} onSelect={setCustomer}/><section className="panel"><div className="section-heading"><div><h3>Recorded purchases</h3><p>Use this to verify customer bookings, referrers and demo transactions. Demo orders are not real payments or entry credentials.</p></div></div><EventTable rows={data.purchases || []} defaultSort="paidAt" defaultDescending columns={[{key:'customer',label:'Customer'},{key:'items',label:'Tickets / packages'},{key:'referredBy',label:'Source'},{key:'salesCents',label:'Total spend',numeric:true,render:(p)=>money(p.salesCents)},{key:'demo',label:'Type',render:(p)=>p.demo ? 'Demo' : 'Paid'},{key:'paidAt',label:'Recorded',render:(p)=>new Date(p.paidAt).toLocaleString()}]}/></section></TabsContent>
